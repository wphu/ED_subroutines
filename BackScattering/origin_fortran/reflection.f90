...
!  z2 = 91 - 92
      ...
      fneps(e,z1,a1,z2,a2) = 0.032534 * e / (z1 * z2 * ( 1.d0 + a1/a2 )	* f(z1,z2))
      rne(th) = r0 + ( 1.d0 - r0 ) / ( 1.d0 + as1 / dtan(th) **	( 2.d0 * as2 ) )
 600  format('	undefined projectile is called	')
      n = 1
      go to 1

      entry reion(theta,energy,nz1,m1,ne,nz2,nw)
	  !write(*,*) "test 1"           !theta,energy,nz1,m1,ne,nz2,nw
...
!	write(*,*)'delta = ',delta
 ...
 40   c = 100.d0 * z1 / z2
      go to 60
 50   c = 5.d0
 60   sb = 61.474d0 / fmu * w * ( dlog ( epsb/(1.d0-beta2) + 1.d0 + c/epsb ) - beta2 ) / fi0 / epsb
      sb = 2.d0 * eps / rangen(eps,energy) + sl * sb / (0.25d0 * sl + sb )
      sbeff = sbeff + wi * sb
 70   continue
      z2 = z2eff / sum
      a2 = a2eff / sum
	  ...
	  if (delta.gt.0.5) then
		redRN = 1.0d0 / ( 1+(eps/0.104d0)**0.577d0 + (eps/0.73d0)**1.5 )
		fmu = a1/a2
		psinum = 1.0d0+24.1d0*fmu**(-3.995d0)
		psiden1 = fmu / ( (1+fmu) * (eps/1.84d0+1.0d0) )
		psiden2 = (1.0d0+fmu)**3 * eps / ( fmu**3 * (1.0d0 - 3.0d0/(2.0d0*fmu) + 0.9d0/fmu**2 + 1.0d0/(2.0d0*fmu**3) ) * (eps+13.3) )   
		psi = psinum / ( psiden1 + psiden2 )

!  sbeff/saeff corresponds to the rho_a/rho-t in the reference paper ( of course sbeff is not equal to rho_a nor is saeff equal to rho_t).

		sklfact = z1**0.66666667d0 / dsqrt(a1) * sb/sa * fmu**2 / (1.0d0+fmu)**2 / psi
		r0heavy = redRN / sklfact
		r = 1.d0 / ( 1.d0 + (eps/0.133d0 ) ** 0.285 ) + 0.530 / ( 1.d0 + (85.0d0/eps) **1.46d0 )
...
!	write(*,*)'did light projectile calculation delta < 0.5 '
      ...
      r0 = r0 / (1.d0 + (eps/0.047d0) ** 0.597d0 + ( eps / 0.619d0 ) **	1.5d0 )
      th = theta * 1.745329d-2
      if (n.eq.1) go to 80
      if (th.eq.0) go to 78
      as1 = 17.9 * eps ** 0.453d0
 ...
 78   reion = r0
 79   return
 80   r = 1.d0 / ( 1.d0 + (eps/0.133d0 ) ** 0.285 ) + 0.530 / ( 1.d0 +	(85.d0/eps) ** 1.46d0 )
...
!  gamma( 0 , x ) = - eibar( - x )	for x < 0
      ...
      data a / 0.2677737343d0 , 8.6347608925 , 18.059016973d0 ,	8.5733287401 /
 ...
 41   expint = 1.d-38
      return
 43   p = (((( z + a(4) ) * z + a(3)) * z + a(2)) * z + a(1) ) /(((( z + b(4) ) * z + b(3)) * z + b(2)) * z + b(1) )
      expint = dexp ( -z ) * p / z
      return
      end
